...
def rule_node(state: PlayerState):
    ...
    # ✅ NameError 해결: user_input 변수 초기화 (함수 시작 부분에서)
    user_input = state.get('user_input', state.get('last_user_input', '')).strip()
    ...
    user_action = user_input  # state.get('last_user_input', '').strip() 대신 user_input 사용
    ...
    if state['parsed_intent'] == 'attack':
        ...
        if npc_state_after and npc_state_after.get('status') == 'dead':
            ...
            if isinstance(npcs_data, dict):
                # 딕셔너리인 경우: {"1": {...}, "2": {...}} 형태
                npc_list = list(npcs_data.values())
                logger.info(f"💰 [LOOT] NPCs data is dict, converted to list: {len(npc_list)} NPCs")
            elif isinstance(npcs_data, list):
                # 리스트인 경우: [{...}, {...}] 형태
                npc_list = npcs_data
            else:
                logger.warning(f"⚠️ [LOOT] Unexpected npcs_data type: {type(npcs_data)}")
            ...
            for npc_data in npc_list:
                ...
                npc_name_in_data = npc_data.get('name', '')

                # NPC 이름이 정확히 일치하는지 확인
                if npc_name_in_data == target_npc:
            ...
            # NPC를 찾지 못한 경우 로그
            if not dropped_items and npc_list:
                logger.warning(f"⚠️ [LOOT] NPC '{target_npc}' not found in scenario npcs data for loot drop")
    ...
    if state['parsed_intent'] == 'item_action':
        ...
        # ✅ 줍기 키워드 확인
        pickup_keywords = ['줍', '습득', '챙긴', '획득', '가져', '집어', '주워', '얻']
        ...
        old_stuck_count = state.get('stuck_count', 0)
        state['stuck_count'] = old_stuck_count + 1
        logger.info(f"📈 [PROGRESS] stuck_count increased: {old_stuck_count} -> {state['stuck_count']} (item_action in rule_node)")
...
def check_npc_appearance(state: PlayerState) -> str:
    ...
    # [FIX] NPC와 적을 모두 처리
    npc_names = scenario.get('npcs', [])
    enemy_names = scenario.get('enemies', [])
    scene_type = scenario.get('type', 'normal')
    scene_title = scenario.get('title', 'Untitled')
