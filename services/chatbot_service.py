...
class ChatbotService:
    ...
    # ▼▼▼ [완전 통합] 키워드 분석 로직 (모든 룰을 리스트로 통합하여 점수 경쟁) ▼▼▼
    @staticmethod
    def get_keyword_response(query: str) -> dict:
        """
        AI 모델 연결 불가 시, 질문의 핵심 단어를 분석하여 준비된 답변을 제공합니다.
        모든 룰을 하나의 리스트에서 점수(Score) 기반으로 경쟁시켜 가장 적합한 답변을 찾습니다.
        """
        ...
        responses = [
            ...
            # 2. 시나리오 제작 방법 [수정: 우선순위 높임]
            {
                "keywords": ['시나리오', '제작', '만들', '생성', '빌더', 'create', '노드', '방법', 'how'],
                "required": ['제작', '만들', '생성', '빌더', '방법'],
            ...
            # 4. 프리셋 vs 시나리오 차이
            {
                ...
                "required": ['프리셋'],  # '시나리오' 단일 검색 시 2번 룰에 지도록 설정
            ...
            # 9. 힌트/막힘
            {
                "keywords": ['막혔', '힌트', 'hint', '모르겠', '뭐해', '어떻게', '할일', '다음'],
                "required": [],
            ...
            # 12. 인기/추천 시나리오 (DB 연동 필요한 경우)
            {
                "keywords": ['인기', '추천', '랭킹', '순위', 'popular', 'top', '1위'],
                "type": "db_popular",  # 특수 타입 마킹
                "answer": "",  # DB 조회 후 채워짐
        ...
        # [매칭 로직] 가장 많은 키워드가 포함된 답변을 선택 (Best Match)
        best_match = None
        max_score = 0
        ...
        for item in responses:
            ...
            # 동점일 경우, 나중에 나온 룰(더 구체적인 룰)이나, 필수단어가 있는 룰을 선호하도록 로직 개선 가능
            if score > max_score:
                max_score = score
                best_match = item
