...
class ChatbotService:
    ...
    def get_keyword_response(query: str) -> dict:
        ...
        if any(w in query for w in ['전투', '공격', '싸움', 'attack', 'fight', '죽이']):
            return {
                "answer": "⚔️ **전투 진행 방법**\n\n적과 조우했을 때 원하는 공격 방식을 구체적으로 묘사하세요.\n\n예시:\n• \"검을 휘둘러 적의 다리를 노린다.\"\n• \"파이어볼 주문을 외운다.\"\n\n나의 행동 -> AI의 판정 및 결과 묘사 -> 적의 반격 순서로 턴이 진행됩니다.",
                "choices": ["주사위는 어떻게 굴려요?", "도망칠 수 있나요?", "아이템 사용법"]
            }

        # 10-3. 저장/불러오기 (인게임)
        if any(w in query for w in ['저장', '세이브', 'save', '불러오기', 'load', '중단']):
            return {
                "answer": "💾 **게임 저장 방식**\n\nTRPG Studio는 **자동 저장(Auto Save)**을 지원합니다.\n\n플레이어가 행동을 입력하고 AI가 답변할 때마다 진행 상황이 서버에 안전하게 저장됩니다.\n언제든지 브라우저를 닫고 나중에 다시 **'이어하기'**를 할 수 있습니다.",
