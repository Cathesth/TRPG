...
class PresetService:
    ...
    @staticmethod
    def save_preset(data: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
        """
        프리셋 저장
    ...
    @staticmethod
    def load_preset(filename: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        """
        프리셋 로드

        Returns:
            (preset_data, error_message)
        """
        if not filename:
    ...
    @staticmethod
    def delete_preset(filename: str) -> Tuple[bool, Optional[str]]:
        """
        프리셋 삭제
