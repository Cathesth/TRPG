...
def list_scenarios():
    ...
    file_infos = ScenarioService.list_scenarios(sort_order, user_id, filter_mode)
    ...
    for info in file_infos:
        fid = info['filename'] # DB ID를 filename으로 사용
        title = info['title']
        desc = info['desc']
        author = info['author']
        ...
        html += f"""
        <div class="bg-gray-800 p-5 rounded-lg border border-gray-700 hover:border-indigo-500 transition-colors flex flex-col justify-between h-full group relative">
            <div>
            ...
            <button hx-post="/api/load_scenario" hx-vals='{{"filename": "{fid}"}}' hx-target="#init-result"
                    class="w-full bg-indigo-900/80 hover:bg-indigo-800 text-indigo-200 py-2 rounded text-sm font-bold flex justify-center gap-2 border border-indigo-800/50">
...
def load_scenario():
    ...
    if error: return f'<div class="text-red-500">로드 실패: {error}</div>'
    ...
    <div class="bg-green-900/30 border border-green-800 text-green-400 p-4 rounded-lg flex items-center gap-3 fade-in mt-4">
        ...
        <div><div class="font-bold">"{scenario.get('title')}" 로드 완료!</div></div>
    </div>
    <button onclick="submitGameAction('시작')" class="mt-3 w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg">
        <i data-lucide="play" class="w-5 h-5"></i> 게임 시작하기
    </button>
