...
# ==========================================
# [교체] routes/api.py의 list_scenarios 함수
@api_router.get('/scenarios', response_class=HTMLResponse)
async def list_scenarios(
        request: Request,
        sort: str = Query('newest'),
        filter: str = Query('public'),  # URL 파라미터로 들어오는 값
        limit: Optional[int] = Query(None),
        user: CurrentUser = Depends(get_current_user_optional)
):
    """
    시나리오 목록을 HTML 카드 형태로 반환합니다. (FastAPI + HTMX)
    index.html의 CSS 클래스(scenario-card-base)와 매칭되어 디자인 깨짐을 방지합니다.
    """
    # 1. 변수명 충돌 방지 및 유저 확인
    filter_mode = filter
    ...
    if not file_infos:
        msg = "아직 생성한 시나리오가 없습니다." if filter_mode == 'my' else "표시할 시나리오가 없습니다."
    ...
    NEW_THRESHOLD = 30 * 60  # 30분 이내 작성글은 NEW

    # 4. HTML 생성 (index.html의 CSS 클래스 강제 적용)
    html = ""
